...
src/
‚îú‚îÄ‚îÄ agents/
‚î?  ‚îî‚îÄ‚îÄ application_guide_generator.py    # Main implementation
‚îî‚îÄ‚îÄ mcp_server/
    ‚îî‚îÄ‚îÄ tools/
...
Tests include:
- ‚ú?Successful guide generation
- ‚ú?No jobs found scenario
- ‚ú?Platform detection (8 platforms)
- ‚ú?Form instruction generation (Greenhouse, Lever, LinkedIn)
- ‚ú?Resume path sanitization
- ‚ú?Default date handling
- ‚ú?Rate limiting verification
- ‚ú?Safety features verification
...
8. **Generate application instructions (Task 8)** ‚Ü?THIS TASK
...
Claude: [Updates database with approvals]
        ...
        ‚ú?Created instructions/apply_jobs_2026-01-29.json
...
‚ú?Queries HIGH and MEDIUM match jobs from database
‚ú?Detects 8+ different job platforms
‚ú?Generates platform-specific form instructions
‚ú?Includes comprehensive safety features
‚ú?Outputs valid JSON for Antigravity agent
‚ú?Has 97% test coverage with 9 passing tests
‚ú?Integrates with MCP server tools
‚ú?Supports standalone CLI usage
‚ú?Includes extensive documentation
